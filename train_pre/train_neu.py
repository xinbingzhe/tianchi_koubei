...
way1 = 'r'
road3 = "E:/tianchi_koubei/result/err_shop_loss_neu_last14.txt"
way2 = 'w'
...
while i<200:
    ...
    params = {'activation':'tanh','solver':'lbfgs','learning_rate':'adaptive','early_stopping':True}
    neu = MLPRegressor(**params)
    neu.fit(x_train,y_train)
    y_pre_neu = neu.predict(x_test)
    ###
    '''
    ...
    neu_pre.append(y_pre_neu)
    Y_test.append(y_test)
    #y_pre_diff = mean_normal_weekend_diff(Y,xday,xweekend,-21,-14)
    
    ###
    loss_neu = Evaluation([y_pre_neu],[y_test])
    if loss_neu>0.07:
        output(fw,i+1,y_pre_neu)
        fw.write(str(i+1)+',neu,'+str(loss_neu)+'\n')
...
fw.close()
#fw_gbrt.close()
print((Evaluation(neu_pre,Y_test)))
