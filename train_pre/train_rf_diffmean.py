...
way1 = 'r'
road3 = "E:/tianchi_koubei/result/err_shop_loss_rf_last14.txt"
road4 ="E:/tianchi_koubei/result/err_shop_loss_diffmean.txt"
way2 = 'w'
...
while i<100:
    ...
    params_rf = {'n_estimators':100,'min_samples_split':2,'warm_start':True,'n_jobs':4}
    ...
    diffmean_pre.append(y_pre_diff)
    ###
    loss_rf = Evaluation([y_pre_rf],[y_test])
    loss_diffmean = Evaluation([y_pre_diff],[y_test])
    if loss_rf>0.07:
        output(fw_rf,i+1,y_pre_rf)
        fw_rf.write(str(i+1)+',rf,'+str(loss_rf)+'\n')
    
    if loss_diffmean>0.07:
        output(fw_diffmean,i+1,y_pre_rf)
        fw_diffmean.write(str(i+1)+',gbrt,'+str(loss_diffmean)+'\n')
