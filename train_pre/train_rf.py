...
way1 = 'r'
road3 = "E:/tianchi_koubei/result/err_shop_loss_rf_last14_0.9.txt"
...
def Evaluation(pred,test):
    ...
    total_err = shop_err/(len(pred)*len(pred[0]))
    return total_err
def create_sampleweight(l):
    sample_weight = []
    for i in range(0,l):
        if i > l-15:
            sample_weight.append(i*0.015)
        else:
            sample_weight.append(i*0.01)
...
while i<10:
    ...
    x_test = X[-7:]
    y_test = Y[-7:]
    #sample_weight = create_sampleweight(len(y_train))
    ###
    '''
    ###
    ...
    ###
     '''
    params_adb = {'n_estimators': 800, 'loss': 'square', 'learning_rate': 0.01,  'random_state': 3}
    ...
    y_pre_br = br.predict(x_test)
    br_pre.append( y_pre_br)



    knn = KNeighborsRegressor(n_neighbors=14,algorithm='brute')
    knn.fit(x_train,y_train)
    y_pre_knn = knn.predict(x_test)
    knn_pre.append(y_pre_knn)
    #print(y_pre_knn)
    #print(y_pre_rcv)
    ...
    if loss_rf>0.05:
        ...
        path = "e://tianchi_koubei/fig/rf_train_7d_0.9_2.0/"+str(i+1)+'.png'
...
print(Evaluation(adb_pre,Y_test))
print(Evaluation(rcv_pre,Y_test))
#print(Evaluation(br_pre,Y_test))
print(Evaluation(knn_pre,Y_test))
