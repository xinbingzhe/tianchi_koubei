...
way1 = 'r'
road3 = "E:/tianchi_koubei/result/err_shop_loss_rf_last14_0.9_val.txt"
...
while i<2000:
    ...
    '''
    ###
    params_ExtraTrees = {'max_features':'log2','n_estimators': 600,'max_depth':10,'oob_score': True, 'n_jobs':4,'bootstrap':True}
    ...
    if loss_rf>0.09:
        ...
        per = np.mean(y_test)/np.mean(y_pre_ext)

    ext.fit(np.concatenate((x_train,x_test),axis=0),np.append(y_train,y_test))
    pre_ori = ext.predict(x_val)
    if per <1:
        y_last_pre = pre_ori*per*1.05
    elif per >1:
        y_last_pre = pre_ori*per*0.95
    else:
        y_last_pre = pre_ori
    
    ori_pre.append(pre_ori)
    ext_pre.append(y_last_pre)
    Y_test.append(y_val)

    loss = Evaluation([y_last_pre],[y_test])
    if loss > 0.09:
        fw_rf.write(str(i+1)+',rf,'+str(loss)+'\n')
    plt.scatter(xday[-(start+14):-28],y_train)
    plt.scatter(xday[-28:-21],y_test,color = 'green')
    plt.plot(xday[-28:-21],y_pre_ext,color = 'red')
    plt.plot(xday[-21:-14],y_last_pre,color = 'yellow')
    plt.plot(xday[-21:-14],pre_ori,color = 'black')
    plt.scatter(xday[-21:-14],y_val,color = 'blue')
    path = "e://tianchi_koubei/fig/rf_train_7d_0.9_val/"+str(i+1)+'.png'
...
print(Evaluation(ext_pre,Y_test))
print(Evaluation(ori_pre,Y_test))
