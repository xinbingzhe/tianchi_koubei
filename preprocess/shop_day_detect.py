...
def compute(num,data,xday,xweekend):
    ...
    if num < 14:
        ...
        for n,w,d in zip(xday[num-14:num],xweekend[num-14:num],data[num-14:num]):
            if w == 0:
                normalday_data.append(d)
            else:
                weekend_data.append(d)
...
while i<2000:
    ...
    for d in range(0,len(pay_day)):
        if pay_day[d]==0 or 0<pay_day[d]<8:
            if d < 14:
                if pay_day[d] < np.mean(pay_day[:14])/1.5:
                    pay_day[d] = compute(d,pay_day,xday[-350:],xweekend[-350:])
            elif 14 <= d < 336:
                if pay_day[d] < np.mean(pay_day[d-14:d+14])/1.5:
                    pay_day[d] = compute(d,pay_day,xday[-350:],xweekend[-350:])
            elif d>= 336:
                if pay_day[d] < np.mean(pay_day[d-14:])/1.5:
                    pay_day[d] = compute(d,pay_day,xday[-350:],xweekend[-350:])
